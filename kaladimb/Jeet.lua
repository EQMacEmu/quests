function event_say(e)
	...
	elseif(e.message:findi("628") or e.message:findi("volunteer") or e.message:findi("run") or e.message:findi("collect")) then
...
function event_trade(e)
	...
	if(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13054, item2 = 13054, item3 = 13054, item4 = 13054},1,text)) then -- Giant Rat Pelt
	...
	elseif(e.other:GetFaction(e.self) < 4 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13282})) then -- Scrap Metal
	...
	elseif(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12167})) then -- Gnome Take (Bad)
	...
	elseif((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 12162}))) then -- Gnome Take (Good)
