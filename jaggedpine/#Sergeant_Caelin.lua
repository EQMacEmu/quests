...
function event_say(e)
	...
	elseif(e.message:findi("shift report")) then
		...
		elseif(e.other:GetFaction(e.self) < 7)then
			e.self:Say("I'm not quite sure I can trust you with that information yet.");
		else
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 2388}, 0)) then -- Qeynos Badge of Honor
	...
	elseif(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8279}, 0)) then -- Stack of Shift Reports
	...
	elseif(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8287}, 0)) then -- Orders for Sergeant Caelin
	...
	elseif(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8286,item2 = 8285})) then -- Closed Black Satchel and Marked Qeynos Badge of Honor
		...
		local canine = item_lib.count_handed_item(e.self, e.trade, {8264});
		if(canine > 0) then
			repeat
