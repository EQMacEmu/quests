...
function event_say(e)
	if(e.message:findi("hail") or e.message:findi("long") or e.message:findi("sacred way") or e.message:findi("faith in my heart") or e.message:findi("willing") or e.message:findi("azure idol of karana")) then
		if(e.other:GetFaction(e.self) < 2) then
			if(e.message:findi("hail")) then
				e.self:Say("Good day to you, " .. e.other:GetCleanName() .. ". I am Annous Pineshadow, high priest of The Rainkeeper and ambassador of Qeynos. It has been a [long] time since I last set foot within the Jaggedpine, and although not surprised, I am amazed and awestruck at the untouched purity of these sacred grounds. It is very much a relief to know that the forest has been sparred the taint of the venomous world that has become Norrath.");
			...
			elseif(e.message:findi("sacred way"))then
			...
			elseif(e.message:findi("faith in my heart"))then
			...
			elseif(e.message:findi("azure idol of karana"))then
	...
	elseif(e.message:findi("prove myself worthy") or e.message:findi("dormant crystal sphere") or e.message:findi("unique to the rainkeeper") or e.message:findi("writ")) then
		if(e.other:GetFaction(e.self) < 2 and eq.get_qglobals(e.self,e.other)["galeweaver"] == "1") then
...
function event_trade(e)
	...
	if(e.other:GetFaction(e.self) < 2 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8956}, 0)) then
	...
	elseif(e.other:GetFaction(e.self) < 2 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8964,item2 = 8963, item3 = 8962})) then -- Writ, sphere, and scroll turn in
		...
		e.other:Faction(e.self,302,2); -- Protectors of Pine
		e.other:Faction(e.self,262,1); -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,17135,10000);
		eq.delete_global("galeweaver");
	elseif(e.other:GetFaction(e.self) < 2 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8966}, 0)) then -- Turn in orb of the gale
		...
		e.other:Faction(e.self,1597,15); -- Residents of Jaggedpine
		e.other:Faction(e.self,272,7); -- Jaggedpine Treefolk
		e.other:Faction(e.self,302,7); -- Protectors of Pine
		e.other:Faction(e.self,262,1); -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,8967,10000);
	end
