function event_say(e)
	...
	elseif(e.message:findi("noble paladin of underfoot")) then
		if(e.other:GetFactionValue(e.self) >= 100) then -- Needs Amiable
			e.self:Say("Yes, you shall do, noble " .. e.other:GetCleanName() .. ". Take this. This is the Chalice Case. It is intended for the Chalice of Conquest and the [Candle of Bravery]. If you should get both, be sure to place them within the Chalice Case and combine them. Take the full Chalice Case to Lord Nightseer in Kaladim and I shall be redeemed and you shall surely earn a fine paladin's reward. Let your quest begin in the mountains of Rathe.");
			e.other:SummonCursorItem(17976); -- Item: Chalice Case - empty
...
function event_trade(e)
	...
	if(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12281})) then
		...
		e.other:Faction(e.self,297, 10); -- Faction: Paladins of Underfoot
