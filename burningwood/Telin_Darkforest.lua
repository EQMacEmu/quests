...
function event_say(e)
	if(e.other:GetFaction(e.self) < 5) then
		...
		elseif(e.message:findi("action")) then
			...
			e.other:SummonCursorItem(20472); -- Item: Worn Note
		end
	else
...
function event_trade(e)
	...
	if(e.other:GetFaction(e.self) < 5) then
		if(item_lib.check_turn_in(e.self, e.trade, {item1 = 20447},0)) then
		...
		elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20468},0)) then
		...
		elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20481},0)) then
			e.self:Emote("smiles slowly. 'Now that's fine work, " .. e.other:GetCleanName() .. ". Just a moment, we have a friend here who can enchant one of the blades. Give her the blade that the ancient smithed.'");
			e.other:QuestReward(e.self,0,0,0,0,20481);
			eq.spawn2(87019,0,0,2852,3239,-155,109.5); -- NPC: Faelin_Bloodbriar
		end
	end
