function event_say(e)
	...
	elseif(e.message:findi("undead")) then
		e.self:Say("I believe the undead prowl these hills at night. I have found the remains of several adventurers who obviously had the misfortune of running into some of Bertoxxulous' dark minions. One young human was still clutching this parchment in his cold, dead fist.");
	elseif(e.message:findi("parchment")) then
		if(e.other:GetFaction(e.self) < 5) then
			...
			e.other:SummonCursorItem(13718);
		end
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18970}, 0)) then
	...
	elseif(eq.get_qglobals(e.self,e.other)["niclaus"] == "1" and item_lib.check_turn_in(e.self, e.trade, {item1 = 13722}, 0)) then
