function event_say(e)
	...
	elseif(e.message:findi("begin")) then
		...
		e.other:SummonCursorItem(3876); -- here there were 3 identical A Heavy Coin all with no lore 3876 3877 abd 3878 after finding no way to distinguish them i chose this one
	elseif(e.message:findi("meaning")) then
	...
	elseif(e.message:findi("with the attacks")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 3877}, 0)) then
		...
		e.other:QuestReward(e.self,0,0,0,0,3878);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 3879}, 0)) then
		e.self:Say("Splendid job, " .. e.other:GetCleanName() .. "! You managed that task quite nicely. Please, keep the coin. The most important part of that task was the fact that you did not stop to question what was being asked of you. That sort of trust is commendable. The Eye is looking for people like you. We need people who display not only integrity, but trust. Trust in our purpose here and you will do well. Out means may seem a bit harsh at times, but we are here to protect our people from an enemy whose methods are without honor. It is one of those very [attacks] on our people that I will need your help with at this time.");
		e.other:QuestReward(e.self,0,0,0,0,3879);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 3882}, 0)) then
		...
		e.other:QuestReward(e.self,0,0,0,0,3881);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 3883}, 0)) then
		...
		e.other:QuestReward(e.self,0,0,0,0,3884);
	end
