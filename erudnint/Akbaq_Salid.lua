...
function event_say(e)
	if (e.other:GetFaction(e.self) <= 5) then --Require indifferent or greater faction    
		if(e.message:findi("hail")) then
			e.self:Say("Greetings, " .. e.other:GetCleanName() .. ". I have recently returned to Erudin since hearing word of the titan spirits that inhabit the Stonebrunt Mountains. Should you be heading in that direction there are some [samples] that I am interested in acquiring."); 
...
function event_trade(e)
	...
	if (e.other:GetFaction(e.self) <= 5) then -- Require indifferent or greater faction 
		if(item_lib.check_turn_in(e.self, e.trade, {item1 = 6960,item2 = 6946})) then -- astral cloak of the titans 
			...
			e.other:Faction(e.self,265,-1); -- Heretics decreased 
			e.other:Faction(e.self,267,1); -- High Guards of Erudin 
