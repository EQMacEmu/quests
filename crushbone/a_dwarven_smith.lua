...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 10351}, 0)) then -- Brass Earring
		...
		e.other:QuestReward(e.self,0,0,0,0,13850,5000); -- Unfinished Blade Mold
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 9307}, 0)) then -- Shiny Brass Shield
		...
		-- Confirmed Live Experience
		e.other:QuestReward(e.self,0,0,0,0,13851,6000); -- Unfinished Blade Mold
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 3301}, 0)) then
		...
		-- Confirmed Live Experience
		e.other:QuestReward(e.self,0,0,0,0,6315,10000); -- Item: Dwarven Mace
		eq.depop_with_timer();
	end
