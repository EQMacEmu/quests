...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		...
		elseif(qglobals["wizepicA"] == "1") then
			...
			e.other:SummonCursorItem(14334); -- Item: Arantir's Ring
		end
	...
	elseif(e.message:findi("gnome") and qglobals["wizepicA"] == "1") then
		...
		e.other:SummonCursorItem(18169); -- Item: Note from Arantir
		e.other:Faction(e.self,404, 30,0); 									--Truespirit
		eq.depop();
	end
end
