...
function event_waypoint_arrive(e)
	...
	if(e.wp > 0 and e.self:GetWaypointPause() > 1) then
		if(rand == 1) then
			e.self:Emote("sniffs the air around them.  Apparently it smells an intruder.");
		else
...
function event_waypoint_depart(e)
	if(e.wp > 0 and e.self:GetWaypointPause() > 1) then
		if(rand == 1) then
			e.self:Emote("gives an indication of indifference and goes back to searching for food.");
		else
