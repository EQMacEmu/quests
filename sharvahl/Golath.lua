...
function event_say(e)
	...
	elseif(e.message:findi("bit")) then
		e.self:Say("Don't let the price fool you, my friend. This is a meal fit for royalty. The giant mushrooms are harvested from the Grimling Forest and marinated in a light payala glaze. Fried to a golden brown, these bits will keep you healthy and well fed.");
	elseif(e.message:findi("pie")) then
		e.self:Say("Our fishermen have returned with the succulent fish found within the Hollowshade Moor. They had to fend off bloodthirsty grimlings to bring you these tasty treats. I've taken the time to bake them into a mouth watering pie. Get em while they're hot!");
	elseif(e.message:findi("bread")) then
		e.self:Say("Some of our Taruun have gone forth and returned with pieces of the giant mushroom trees in Grimling Forest. It has a distinct texture and flavor that is delicious when baked into our lightly sweetened bread. Try it, I'm sure you'll like it.");
	elseif(e.message:findi("bone")) then
		e.self:Say("Despite being tricky to eat, the bone chews are quite popular. Just bite off the end and suck out the marrow. It'll make you strong, try them.");
	elseif(e.message:findi("stuffed")) then
		e.self:Say("Ahh yes, my favorite. The tender meat of the owlbear makes an excellent stuffing for the giant mushrooms found within the Grimling Forest. Healthy, filling, and pleasing to taste. An excellent meal, try it.");
