function event_say(e)
	...
	elseif(e.message:findi("fish")) then
		e.self:Say("Our fishermen have worked hard to catch the fattest and juiciest fish in the Hollowshade Moor. They are salted to preserve them on your long journeys. Try them!");
	elseif(e.message:findi("almond")) then
		e.self:Say("The almonds are great to snack on during your many journeys. I get up early every day to select and prepare the very best ones. I hope you like them.");
	elseif(e.message:findi("shawerma")) then
		e.self:Say("You have a good eye my friend. The Shawerma is made from fresh owlbear meat carefully prepared in spices and Nepeta oil. It's very tender and delicious. Try some, you'll love them.");
	elseif(e.message:findi("nepeta")) then
		e.self:Say("Nepeta Oil is useful for many many things. You can also drink it but it can be very intoxicating.");
	elseif(e.message:findi("mint")) then
		e.self:Say("I'm just crazy for these mints, something about them just makes me feel so alive. WOO!");
	elseif(e.message:findi("meatcake")) then
		e.self:Say("These are perfect for travelers on a budget. The marrow is extracted from owlbear marrow fresh every day. They are cooked in Nepeta oil and fashioned into bite sized patties for your convenience. Healthy and tasty, a perfect treat.");
	elseif(e.message:findi("payala")) then
		e.self:Say("I got the payala from Saren. I help her collect them sometimes, they are sweet and delicious. I cut them into slices and bake them in preservative spices to help make them last on those long journeys.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 30625, item2 = 30625, item3 = 30625, item4 = 30625})) then
		e.self:Say("Oh wow! This is just what I was hoping for. Thank you so much! You must be a real good friend to Saren to have her make you these, she never makes freshly squeezed nectar unless she really likes someone. You have a kind heart " .. e.other:GetCleanName() .. ". Please take this with you as a token of my friendship.");
		e.self:Emote("smiles as she hands " .. e.other:GetCleanName() .. " a shimmering emberstone ring.");
		e.other:QuestReward(e.self,0,0,0,0,30609,1000);
	end
