...
function event_timer(e)
	...
	if(count == 2) then
		eq.signal(155122,1,1); -- NPC: Taruun_Jaur
	end
	if(count == 3) then
		eq.signal(155259,1,1); -- NPC: Erat_Almar
	end
	if(count == 4) then
		eq.signal(155122,2,1); -- NPC: Taruun_Jaur
	end
	if(count == 5) then
		e.self:Say("Sure thing, mate. You know I'm sucker for good gossip.");
	end
	if(count == 6) then
		eq.signal(155122,3,1); -- NPC: Taruun_Jaur
	end
	if(count == 7) then
		e.self:Say("Oh my! Did ya kill 'em!");
		eq.signal(155259,2,1); -- NPC: Erat_Almar
	end
	if(count == 8) then
		eq.signal(155122,4,1); -- NPC: Taruun_Jaur
	end
	if(count == 9) then
