function event_say(e)
	...
	elseif(e.message:findi("snake")) then
		e.self:Say("The snake is prepared in delicious spices and blackened over a grill with peppers and nepeta oil. It is a delicious delicacy.");
	elseif(e.message:findi("cake")) then
		e.self:Say("Our cakes are perfect to celebrate your joyous occasions. A slight hint of rum gives them a delicious flavor.");
	elseif(e.message:findi("kabob")) then
		e.self:Say("The kabobs are made of the most tender portions of owlbear meat. They are marinated overnight in spices and payala nectar, then grilled over a spit until they reach perfection. A favorite during festivities.");
	elseif(e.message:findi("chocolate")) then
		e.self:Say("It is a dark mint chocolate, very delicious. Just don't give too many to your offspring or they will be bouncing off of the walls.");
	elseif(e.message:findi("mussel")) then
		e.self:Say("Great choice, the mussels are boiled thoroughly in water along with thin lemon slices and our best white wine. Once ready we add a prepared mixture of spices, including saffron. It is garnished with almonds and leak and covered lightly with a specially made white wine sauce. They are very delicious, try some.");
	elseif(e.message:findi("bass")) then
		e.self:Say("Our sea bass is caught fresh every day by fishermen who brave the journey to the twilight sea. It is prepared in spices and seared to perfection. My personal favorite, I recommend them.");
	elseif(e.message:findi("feast")) then
		e.self:Say("A wounderful cornucopia of steamed vegetables. Healthy and filling, it is a popular dish for many visitors of our establishment.");
